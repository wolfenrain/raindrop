...
import 'package:raindrop_postgres_example/database/migrations.dart';
...
void main() async {
  ...
  // Apply any migration that has not run yet, generated by `raindrop_cli`.
  ...
  final userIds =
      await db.select(users.id).from(users).orderBy({users.id: Order.asc});
  final firstUserId = userIds.first!;

  await db.insert(into: pets).values([
    Pet(name: 'Rex', ownerId: firstUserId),
    Pet(name: 'Milo', ownerId: firstUserId),
    Pet(name: 'Smokey', ownerId: user.id!),
  ]);
  ...
  final usersFound = await db.select().from(users).where(
        not(users.id.equals(firstUserId)) & users.name.like('%est%'),
      );
  ...
  final updatedNames = await db
      ...
      .where(users.id.equals(firstUserId))
      .returning();
