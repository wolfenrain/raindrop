...
CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, "favoriteGame" TEXT NOT NULL, age INTEGER NOT NULL, is_active INTEGER NOT NULL, rating REAL NOT NULL, "deletedAt" INTEGER)''',
  ...
  test('column arithmetic: subtract, multiply, divide', () async {
    final row = (await db
            .select(users.age - 1, users.age * 2, users.rating / 2)
            .from(users)
            .limit(1))
        .single;
    expect(row.$1, 29);
  ...
  test('integer modulo evaluates and filters', () async {
    final remainders = await db.select(users.age % 7).from(users);
    expect(remainders, [2, 0, 6]);

    final even = await db.select(users.name).from(users).where(
          (users.age % 2).equals(0),
        );
    expect(even, containsAll(['Morgan', 'Alex']));
  });
}
