...
void main() {
  ...
  setUp(() async {
    ...
    await db.execute(
      '''
CREATE TABLE pets (id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id INTEGER NOT NULL, name TEXT NOT NULL, rank TEXT)''',
    );
  ...
  group('typing', () {
    ...
    test('one-column selects are unchanged', () async {
      final owners = await db.select(pets.ownerId).from(pets).orderBy({
    ...
    test('a record-typed single column still resolves to Derived1', () {
      ...
      final d = db.select(inner).from(pets).derived(as: 'one');
  ...
  group('naming', () {
    ...
    test('a generated name steps around one already in use', () {
      ...
      expect(
        sql,
        isNot(contains('COUNT("id") AS "c1"')),
        reason: 'the aggregate must take a name other than the taken c1',
      );
    });
  ...
  CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, "favoriteGame" TEXT NOT NULL, age INTEGER NOT NULL, is_active INTEGER NOT NULL, rating REAL NOT NULL, "deletedAt" INTEGER)''',
    ...
    test('renders as a named statement in FROM', () {
      ...
      final (sql, _) = SQLiteDialect().translate((query as ToQuery).compile());
