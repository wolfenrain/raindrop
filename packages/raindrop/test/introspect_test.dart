  ...
  _PetSchema(super.$)
      : id = $.text('id', (p) => p.id).primaryKey(),
        ...
        friendly = $.custom<bool, String, bool>(
          'friendly',
          (p) => p.friendly,
          transformer: const _YesNoTransformer(),
          sqlType: 'TEXT',
          defaultValue: true,
        ),
        // A default the DATABASE evaluates, not a value we supply.
...
final owners = sqliteTable('owners', _OwnerSchema.new);

final pets = sqliteTable('pets', _PetSchema.new, (t) {
...
void main() {
  ...
  Map<String, Object?> snapshot() =>
      buildSnapshot([owners, pets], dialect: dialect, dialectName: 'sqlite');
  ...
  group('the document', () {
    test('carries the identity fields the CLI requires', () {
      ...
      expect(document['dialect'], 'sqlite');
